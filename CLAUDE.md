...
- **Global Constraint: PPT Output Path** — When generating PPT for any project, the final `.pptx` file MUST be saved to the project's `PPT/` subdirectory (i.e., `<project_path>/PPT/<project_name>_<timestamp>.pptx`). This ensures all project artifacts remain self-contained within the project directory. See SKILL.md Global Execution Discipline Rule 10.
...
python3 skills/ppt-master/scripts/svg_to_pptx.py <project_path> -o <project_path>/PPT/<name>_<timestamp>.pptx
