...
> **Output:** Native-shapes `.pptx` (directly editable) saved to `<project_path>/PPT/<name>_<timestamp>.pptx` per the project's global constraint. A copy of `svg_output/` is always snapshotted to `backup/<timestamp>/svg_output/` for re-export / archival. Pass `--svg-snapshot` to additionally emit an SVG-image preview pptx alongside the native pptx (see [FAQ](./docs/faq.md)). Requires Office 2016+.
